...
const CampaignsPage = () => {
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/api/funding-campaign/');
          ...
          >
            ...
            <div className="w-full bg-gray-200 rounded-full h-2.5 mb-4">
              <div
                ...
                style={{ width: campaign.funding_progression || '0%' }}
              ></div>
            </div>

            <div className="flex justify-between text-sm">
              <p className="text-green-600">Raised: ₦{campaign.target}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4">
            ...
            <div className="w-full bg-gray-200 rounded-full h-2.5 mb-6">
              <div
                className="bg-green-500 h-2.5 rounded-full"
                style={{ width: selectedCampaign.funding_progression || '0%' }}
              ></div>
            </div>
